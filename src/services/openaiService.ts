...
export class OpenAIService {
  ...
  private static initializeOpenAI(): OpenAI | null {
  ...
  static async getCarRecommendation(answers: Answer[]): Promise<CarRecommendation> {
    try {
  ...
  private static getMockRecommendation(answers: Answer[]): CarRecommendation {
    // Extract key preferences from answers
    const budget = answers.find(a => a.questionId === 'budget')?.answer as string;
    const usage = answers.find(a => a.questionId === 'usage')?.answer as string;
    const fuelPreference = answers.find(a => a.questionId === 'fuel_preference')?.answer as string;
    const features = answers.find(a => a.questionId === 'features')?.answer as string | string[];
    const brand = answers.find(a => a.questionId === 'brand_preference')?.answer as string;
...
בהתאם להעדפתך, נבחר דגם של ${brand}.`;
  ...
  private static buildPrompt(answers: Answer[]): string {
...
ההמלצה צריכה להיות רלוונטית, מעשית, ובטווח התקציב.`;
  ...
  private static parseRecommendation(response: string): CarRecommendation {
    try {
